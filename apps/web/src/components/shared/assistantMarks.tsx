/**
 * Brand marks for the AI assistants SuperMCP connects to.
 *
 ...
 *
 * Never hand-draw one of these. A mark that is close but wrong reads as a
 * counterfeit of somebody else's brand; if Simple Icons has no entry, use a doodle.
 */
...
/** The four the connect and hero rows advertise, in the order they are shown. */
export const ADVERTISED_ASSISTANTS = [
  ASSISTANT_MARKS.claude,
  ASSISTANT_MARKS.chatgpt,
  ASSISTANT_MARKS.gemini,
  ASSISTANT_MARKS.cursor,
] as const;

export function AssistantMarkIcon({
  mark,
  className,
}: {
  mark: AssistantMark;
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden className={`shrink-0 ${className ?? ""}`}>
      <path d={mark.d} fill={mark.color} fillRule={mark.fillRule} />
    </svg>
  );
}
