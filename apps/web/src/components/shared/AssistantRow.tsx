 ...
 *
 * Used by both the hero and the connect section, which previously duplicated a
 * hand-written list of names.
 *
 * Marks come from Simple Icons (https://simpleicons.org), whose SVGs are CC0.
 * The trademarks themselves belong to Anthropic, OpenAI, Google and Anysphere;
 * they appear here only to say what this connects to, which is what the
 * surrounding label claims and nothing more. Each mark uses that brand's own
 * colour so a shopper recognises the platform at a glance. Fill is set inline
 * (not currentColor) so a parent `text-ink` cannot turn Cursor's orange black.
 *
...
const ASSISTANTS = [
  ...
  {
    name: "Cursor",
    // Cursor Orange from their brand guidelines. The Simple Icons hex is still
    // #000000; we paint the cube ourselves so it does not read as a black blob.
    color: "#F54E00",
        ...
        {ASSISTANTS.map((a) => (
          ...
          >
            <svg
              viewBox="0 0 24 24"
              aria-hidden
              className={`shrink-0 ${compact ? "size-3.5" : "size-[1.05rem]"}`}
              style={{ fill: a.color }}
            >
              <path
                d={a.d}
                fill={a.color}
                fillRule={"fillRule" in a ? a.fillRule : undefined}
              />
            </svg>
            <span
              dir="ltr"
