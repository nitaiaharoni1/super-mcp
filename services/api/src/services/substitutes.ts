...
import { getProductById, type ProductSummary } from "./products.js";
...
function appendLocationFilter(
  opts: { city?: string; near?: GeoPoint; radiusKm?: number },
  params: unknown[],
): { whereSql: string; distanceSelect: string } {
  ...
  if (opts.near) {
    ...
    const radius = resolveRadiusKm(opts.near, opts.radiusKm);
    if (radius != null) {
      params.push(radius);
      conditions.push(
        `st.lat IS NOT NULL AND st.lng IS NOT NULL AND ${distanceExpr} <= $${params.length}`,
      );
    }
  }
...
export async function suggestSubstitutes(
  ...
  const baseLoc = appendLocationFilter(opts, baseParams);
  ...
  const loc = appendLocationFilter(opts, params);
  params.push(limit * 8);
